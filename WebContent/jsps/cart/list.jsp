	...
	$(".jian").click(function(){
		...
		if(quantity ==1 ){
			if(confirm("confirm to delete?")){
				location = "/goodsPrototype/CartItemServlet?method=batchDelete&cartItemIds="+id;
			}
		} else{
...
function sendUpdateQuantity(id, quantity){
	$.ajax({
		...
		type: "POST",
		dataType: "json",
		data: {method: "updateQuantity", quantity: quantity, cartItemId: id},
		url: "/goodsPrototype/CartItemServlet",
		success: function(result){
			//1. update quantity
			$("#"+id+"Quantity").attr("value", result.quantity);
			//2. update subtotal
...
function showTotal(){
	...
	$(":checkbox[name=checkboxBtn][checked=true]").each(function(){
		//2.获取复选框的值，即cartItemId,即其他元素的前缀
		var id = $(this).val();
...
function setJieSuan(bool){
	if(bool){
		...
		$("#jiesuan").click(function() {return false}); //使click无效
	}
}
...
function batchDelete(){
	...
	location = "/goodsPrototype/CartItemServlet?method=batchDelete&cartItemIds="+cartItemIdArray;
}
